...
def get_splitter_for_file(file_path: str):
    """
    Get optimal splitter based on file extension.
    ...
    if language:
        try:
            ...
            logger.debug(f"CodeSplitter failed for {language}: {e}")
    ...
    def chunk_document(self, document: Document) -> List:
        ...
        if self.strategy == ChunkingStrategy.HYBRID:
            ...
            try:
                ...
                logger.debug(f"Hybrid chunker falling back to text: {e}")
                return self._text_splitter.get_nodes_from_documents([document])
    ...
    def chunk_documents(self, documents: List[Document]) -> List:
        ...
        for doc in documents:
            try:
                ...
                logger.debug(f"Failed to chunk document: {e}")
                # Fallback to text splitter (max one fallback to prevent recursion)
